...
config.read('/Users/user/PycharmProjects/coincommunity/crawler.conf')


def to_elastic(data):

    pathlink = "/Users/user/PycharmProjects/coincommunity"
        ...
        for day in days:
            if len(str(day)) == 1:
                for each in range(1, 1500):
                    try:
                        es.delete(index="serachnews", doc_type="naver_news", id=del_date[:8] + '0' +str(day) + "-" + str(each))
                    except:
                        pass
            else:
                for each in range(1, 1500):
